...
class BackupTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / "PROFILE.json").write_text('{"name":"before"}', encoding="utf-8")
        (self.root / "CV_MASTER.json").write_text('{"identity":{"name":"Ada"}}', encoding="utf-8")
        (self.root / "source_cv.pdf").write_bytes(b"%PDF-source")
        (self.root / "cv").mkdir()
        (self.root / "cv" / "resume.pdf").write_bytes(b"%PDF-result")
    ...
    def test_archive_contient_manifeste_checksums_et_aucun_secret(self):
        ...
        self.assertIn("CV_MASTER.json", names)
        self.assertIn("cv/resume.pdf", names)
    ...
    def test_restauration_remet_base_et_fichiers_en_place(self):
        info = backup.create_backup()
        archive = backup.backup_path(info["name"])
        (self.root / "PROFILE.json").write_text('{"name":"after"}', encoding="utf-8")
        (self.root / "cv" / "resume.pdf").unlink()
        with store._LOCK, store._connect() as conn:
            conn.execute("DELETE FROM runs")
            conn.execute("DELETE FROM results")
        result = backup.restore_backup(archive)
        self.assertTrue(result["restored"])
        self.assertEqual((self.root / "PROFILE.json").read_text(encoding="utf-8"), '{"name":"before"}')
        self.assertEqual((self.root / "cv" / "resume.pdf").read_bytes(), b"%PDF-result")
